...
struct DirChanges{
    std::vector<const std::string> removed_files;
    std::vector<FileModification> modified_files;
    std::vector<const std::string> created_files;
};
...
FileModification get_file_modification(const FileSnapshot &file_snap_1, const FileSnapshot &file_snap_2);
