...
public:
    FileIO(const std::string &filepath, std::ios::openmode mode = std::ios::in);
    bool open_file(const std::string &filepath, std::ios::openmode mode);
    void close_file();
    ...
    // void replace_chunk_at_range(const size_t start, const size_t end, const std::string &data);
